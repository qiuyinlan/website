...
- 主界面只显示当前微习惯，字号很大
- 顶部有“主线”按钮，可管理多条主线、排序，并分别填写详细拆解
- 顶部有“原则”按钮，可管理多个原则板块、排序，并给每个板块添加多个原则点
- 顶部有“兴趣”按钮，可管理自己真正感兴趣的事情，并支持排序
- 点击“完成”后自动跳到下一个
- 支持“跳过”
- 设置页可以新增、停用、删除、排序微习惯
- 默认本地模式可直接使用
...
2. 在项目里执行 `supabase/schema.sql`
   - 如果你之前已经执行过旧版脚本，现在增加了“主线/原则/兴趣”同步，重新执行一次最新的 `schema.sql` 就行
